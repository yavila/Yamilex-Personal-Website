...
;(function(window) {
	...
	var bodyEl = document.body,
		...
		// lockScroll = false, xscroll, yscroll,									REMOVED
		...
		// zoomCtrl = sliderEl.querySelector('.button-zoom'),						REMOVED
		// the main content element
		// contentEl = document.querySelector('.content'),							REMOVED
		// close content control
		// closeContentCtrl = contentEl.querySelector('button.button-close'),    NOT USING THIS BUTTON
	...
	function initEvents() {
		// open items
		// zoomCtrl.addEventListener('click', function() {							REMOVED
		// 	openItem(items[current]);
		// });

		// close content
		// closeContentCtrl.addEventListener('click', closeContent); 				NOT USING
		...
		window.addEventListener('resize', throttle(function(ev) {
			...
			items.forEach(function(item, pos) {
				...
				var el = item.querySelector('.slide_mover');
	...
	function openItem(item) {
		...
		// var zoomer = item.querySelector('.zoomer');								REMOVED
		// slide screen preview
		// classie.add(zoomer, 'zoomer-active');									REMOVED
		// disallow scroll
		...
		onEndTransition(zoomer, function() {
			...
			classie.add(contentItem, 'content_item-current');
			classie.add(contentItem, 'content_item-reset');
	...
	// 	var contentItem = contentEl.querySelector('.content_item-current'),
	// 		zoomer = items[current].querySelector('.zoomer');
	//
	// 	classie.remove(contentEl, 'content-open');
	// 	classie.remove(contentItem, 'content_item-current');
	...
	// 		classie.remove(this, 'content_item-reset');
	//
	...
	function applyTransforms(el, nobodyscale) {
		...
		var zoomerArea = el.querySelector('.zoomer_area'),
	...
	function navigate(dir) {
		var itemCurrent = items[current],
			currentEl = itemCurrent.querySelector('.slide_mover'),
			currentTitleEl = itemCurrent.querySelector('.slide_title'),
			currentTagline = itemCurrent.querySelector('.slide_tagline'),
			currentButton = itemCurrent.querySelector('.slide_button');
			// currentTags = itemCurrent.querySelector('.slide_tags');
		...
		var itemNext = items[current],
			nextEl = itemNext.querySelector('.slide_mover'),
			nextTitleEl = itemNext.querySelector('.slide_title'),
			nextTagline = itemNext.querySelector('.slide_tagline'),
			nextButton = itemNext.querySelector('.slide_button');
			// nextTags = itemNext.querySelector('.slide_tags');
		...
		// animate current tagline out
		dynamics.animate(currentTagline, { translateX: dir === 'right' ? -250 : 250, opacity: 0 }, {
			type: dynamics.bezier,
			points: [{"x":0,"y":0,"cp":[{"x":0.2,"y":1}]},{"x":1,"y":1,"cp":[{"x":0.3,"y":1}]}],
			duration: 450
		});

		// animate current button out
		dynamics.animate(currentButton, { translateX: dir === 'right' ? -250 : 250, opacity: 0 }, {
			type: dynamics.bezier,
			points: [{"x":0,"y":0,"cp":[{"x":0.2,"y":1}]},{"x":1,"y":1,"cp":[{"x":0.3,"y":1}]}],
			duration: 450
		});

		// // animate the current tags out
		// dynamics.animate(currentTags, { translateX: dir === 'right' ? -250 : 250, opacity: 0 }, {
		// 	type: dynamics.bezier,
		// 	points: [{"x":0,"y":0,"cp":[{"x":0.2,"y":1}]},{"x":1,"y":1,"cp":[{"x":0.3,"y":1}]}],
		// 	duration: 450
		// });
		...
		// set the right properties for the next tagline to come in
		dynamics.css(nextTagline, { translateX: dir === 'right' ? 250 : -250, opacity: 0 });
		// animate the next tagline in
		dynamics.animate(nextTagline, { translateX: 0, opacity: 1 }, {
			type: dynamics.bezier,
			points: [{"x":0,"y":0,"cp":[{"x":0.2,"y":1}]},{"x":1,"y":1,"cp":[{"x":0.3,"y":1}]}],
			duration: 650
		});
		...
		// set the right properties for the next tagline to come in
		dynamics.css(nextButton, { translateX: dir === 'right' ? 250 : -250, opacity: 0 });
		// animate the next tagline in
		dynamics.animate(nextButton, { translateX: 0, opacity: 1 }, {
			type: dynamics.bezier,
			points: [{"x":0,"y":0,"cp":[{"x":0.2,"y":1}]},{"x":1,"y":1,"cp":[{"x":0.3,"y":1}]}],
			duration: 650
		});

		// // set the right properties for the next tags to come in
		// dynamics.css(nextTags, { translateX: dir === 'right' ? 250 : -250, opacity: 0 });
		// // animate the next tags in
		// dynamics.animate(nextTags, { translateX: 0, opacity: 1 }, {
		// 	type: dynamics.bezier,
		// 	points: [{"x":0,"y":0,"cp":[{"x":0.2,"y":1}]},{"x":1,"y":1,"cp":[{"x":0.3,"y":1}]}],
		// 	duration: 650
		// });
